...
export async function scrapeProduct(ProductUrl: string) {
    ...
    try {
        ...
        const currentPrice = extractPrice($('.priceToPay'));
        const ogPrice = extractPrice($('.basisPrice'));
        const outOfStock = $('#availability').text().trim().toLocaleLowerCase() === 'OUT OF STOCK';
        const productImages = $('#landingImage').attr('data-a-dynamic-image') || '{}'
        const productImagesUrls = Object.keys(JSON.parse(productImages));
        console.log({ productName, currentPrice, ogPrice, outOfStock, productImagesUrls });
